...
summary_stats <- dd %>%
  ...
  mutate(range_mean = ifelse(abs(mean) >= (max_minus_min/2),
...
```{r}
## plot magnitude against balance for 8 species using the summary_stats
summary_stats %>%
  ggplot(aes(x = balance, y = magnitude, col = range_mean)) +
  geom_point() +
...
rdiv_a<-data.frame('data' = d1a$x, 'sp.1' = d1a$.derivative, 'sp.2' = d2a$.derivative, 'sp.3' = d3a$.derivative)
...
rdiv_b<-data.frame('data' = d1b$x, 'sp.1' = d1b$.derivative, 'sp.2' = d2b$.derivative, 'sp.3' = d3b$.derivative)
...
rdiv_c<-data.frame('data' = d1c$x, 'sp.1' = d1c$.derivative, 'sp.2' = d2c$.derivative, 'sp.3' = d3c$.derivative)
...
rdiv_d<-data.frame('data' = d1d$x, 'sp.1' = d1d$.derivative, 'sp.2' = d2d$.derivative, 'sp.3' = d3d$.derivative)
...
rdiv_e<-data.frame('data' = d1e$x, 'sp.1' = d1e$.derivative, 'sp.2' = d2e$.derivative, 'sp.3' = d3e$.derivative)
...
rdiv_f<-data.frame('data' = d1f$x, 'sp.1' = d1f$.derivative, 'sp.2' = d2f$.derivative, 'sp.3' = d3f$.derivative)
...
# panel d
Fig_2d<-ggplot(data = NULL,mapping = aes(x = d1a$x,y = d1a$.derivative)) +
  theme_classic(base_size = 14) + 
...
# panel e
Fig_2e<-ggplot(data = NULL,mapping = aes(x = d1b$x,y = d1b$.derivative)) +
  theme_classic(base_size = 14) + 
...
# panel f
Fig_2f<-ggplot(data = NULL,mapping = aes(x = d1c$x,y = d1c$.derivative)) +
  theme_classic(base_size = 14) + 
...
# panel d
Fig_3d<-ggplot(data = NULL,mapping = aes(x = d1d$x,y = d1d$.derivative)) +
  theme_classic(base_size = 14) + 
...
# panel e
Fig_3e<-ggplot(data = NULL,mapping = aes(x = d1e$x,y = d1e$.derivative)) +
  theme_classic(base_size = 14) + 
...
# panel f
Fig_3f<-ggplot(data = NULL,mapping = aes(x = d1f$x,y = d1f$.derivative)) +
  theme_classic(base_size = 14) + 
...
rdiv_r<-data.frame('temperature' = d1_r$temperature,
...
rdiv_K<-data.frame('temperature' = d1_K$temperature,
...
# panel c
Fig_4c<-ggplot(data = d1_r,mapping = aes(x = temperature,y = .derivative)) +
  theme_classic(base_size = 14) + 
...
# panel d
Fig_4d<-ggplot(data = d1_K,mapping = aes(x = temperature,y = .derivative)) +
  theme_classic(base_size = 14) + 
...
coef_plot <- mods %>%
  ...
  labs(title = paste0(study_oi, " (community stability)"), ) +
...
coef_plot2 <- mods %>%
  ...
  geom_point() +
  geom_pointrange(aes(ymin = conf.low, ymax = conf.high),fatten = 5)+
  labs(title = paste0(study_oi, " (covariance)")) +
  xlab("Measure of response diversity") +
  ylab("Relationship with community stability \n(model slope)") +
  facet_grid(cols = vars(trait_name))
coef_plot2
...
And now we look at these same correlations by assessing goodness-of-fit (R-squared):
