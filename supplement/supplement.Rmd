...
Fig_4a<-gam_data_r %>% 
  ...
  geom_ribbon(aes(ymin=Sp1-Sp1_se, ymax=Sp1+Sp1_se),alpha=.2) +
  geom_ribbon(aes(ymin=Sp2-Sp2_se, ymax=Sp2+Sp2_se),alpha=.2) +
  geom_ribbon(aes(ymin=Sp3-Sp3_se, ymax=Sp3+Sp3_se),alpha=.2) +
  geom_ribbon(aes(ymin=Sp4-Sp4_se, ymax=Sp4+Sp4_se),alpha=.2) +
...
rdiv <- rdiv_r %>%
  ...
  summarise(r_dissimilarity = resp_div(derivative,sign_sens = FALSE),
            r_divergence = resp_div(derivative,sign_sens = TRUE)) %>%
  group_by(microcosm_ID) %>%
  summarise(r_dissimilarity = mean(r_dissimilarity),
            r_divergence = mean(r_divergence),
...
Kdiv <- rdiv_K %>%
  ...
  summarise(K_dissimilarity = resp_div(derivative,sign_sens = FALSE),
            K_divergence = resp_div(derivative,sign_sens = TRUE)) %>%
  group_by(microcosm_ID) %>%
  summarise(K_dissimilarity = mean(K_dissimilarity),
            K_divergence = mean(K_divergence),
            temperature = mean(temperature))
...
ggplot(comm_data) +
  geom_point(aes(x = r_cor, y=r_divergence))
ggplot(comm_data) +
  geom_point(aes(x = k_cor, y=K_dissimilarity))
ggplot(comm_data) +
  geom_point(aes(x = k_cor, y=K_divergence, col = composition_code,
