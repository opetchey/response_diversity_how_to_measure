...
* The spread of the distribution. Here we use an entropy index based on hill numbers which can account for both the richness of the communities and their evenness in terms of their functional responses (their derivatives). See chapter 6 here for properties: <https://arxiv.org/pdf/2012.02113.pdf>.

[I don't understand the maths well enough to fill in a meaningful formula here, so someone with a better grasp of hill numbers might want to expand on this...]
...
summary_stats <- dd %>%
  ...
  summarise(entropy = resp_div(rd_vals,sign_sens = FALSE),
...
When only number of species varies, entropy increases.
...
When species are added with identical responses to species already in the community, entropy does not change.That is, redundancy does not inflate response diversity (another desirable property)
...
redundant<-data.frame('S'=num_spp %>% rep(times=c(2,4,8,16,3)),'first_deriv_vals'=NA,'entropy'=NA,'sign_sensitive_range'=NA)
...
for(i in 1:length(redundant$S)){
  ...
  redundant$entropy[i]<-first_deriv_vals %>% resp_div(sign_sens = F)
...
redundant$variable<-redundant$variable %>% 
  as.character() %>% parse_factor(levels = c('first_deriv_vals','entropy','sign_sensitive_range'))
...
Fig_2ga<-ggplot(data = rdiv_a,mapping = aes(x = data,y = rdiv)) +
  theme_classic(base_size = 14) + 
  labs(x = NULL,y = "Entropy (derivatives)",tag = "g)") + 
...
Fig_2ha<-ggplot(data = rdiv_b,mapping = aes(x = data,y = rdiv)) +
  theme_classic(base_size = 14) + 
  labs(x = NULL,y = "Entropy (derivatives)",tag = "h)") + 
...
Fig_2ia<-ggplot(data = rdiv_c,mapping = aes(x = data,y = rdiv)) +
  theme_classic(base_size = 14) + 
  labs(x = NULL,y = "Entropy (derivatives)",tag = "i)") + 
...
Fig_4ea<-ggplot(data = rdiv_r,mapping = aes(x = temperature,y = rdiv)) +
  theme_classic(base_size = 14) + 
  labs(x = NULL,y = "Entropy (derivatives)",tag = "e)") + 
...
Fig_4fa<-ggplot(data = rdiv_K,mapping = aes(x = temperature,y = rdiv)) +
  theme_classic(base_size = 14) + 
  labs(x = NULL,y = "Entropy (derivatives)",tag = "f)") + 
