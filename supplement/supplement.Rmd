...
Get the derivative based response diversity for each of the two species communities at each temperature range. Done by calculating response diversity at each temperature within a range, and then taking the average.

```{r}
## prepare a data frame to put the derivatives into
...
## put the r derivatives into the above data frame and calculate the
## derivative based response diversities
rdiv <- rdiv_r %>%
...
## put the K derivatives into the above data frame and calculate the
## derivative based response diversities
Kdiv <- rdiv_K %>%
  select(-rdiv, -sign, -Med) %>% 
  pivot_longer(names_to = "species", values_to = "derivative", 2:5) %>%
