...
# Two dimensions of response diversity - Entropy
...
summary_stats <- dd %>%
  group_by(num_spp, max_minus_min, mean) %>%
  summarise(range = resp_div(rd_vals, stat = rd_stat, sign_sens = FALSE),
            entropy = resp_fd(rd_vals),
            sign_sensitive_entropy = (entropy - resp_fd(abs(rd_vals))) / entropy,
            sign_sensitive_range = resp_div(rd_vals,
                                            stat = rd_stat,
                                            sign_sens = TRUE)) %>%
  pivot_longer(names_to = "variable", values_to = "value", 4:7)
...
(ref:XXX11) Communities varying only in the range of trait values. Trait values overlap zero. but the extent of overlap is constant, number of species is constant, and mean trait value is constant. Each column of dots corresponds to one community. Top panel shows the values of the first derivative of each of the species in the community. Bottom panel is the measure of entropy.

```{r XXX11,  include=TRUE, fig.cap='(ref:XXX6)', fig.align="center", fig.height=4, fig.width=4}
this_mean <- 0
this_num_spp <- 8
all_data %>%
  filter(mean == this_mean,
...
(ref:XXX12) Communities varying only in the range of trait values, with no overlap of trait values with zero. Extent of overlap of trait values with zero is constant, number of species is constant and mean trait value is constant. All else is as in the previous figure.

```{r XXX12,  include=TRUE, fig.cap='(ref:XXX7)', fig.align="center", fig.height=4, fig.width=4}
this_mean <- 2.5
this_num_spp <- 8
all_data %>%
  filter(mean == this_mean,
...
(ref:XXX13) Communities varying only in the mean trait value, but not in range of trait values or the number of species.

```{r XXX13,  include=TRUE, fig.cap='(ref:XXX8)', fig.align="center", fig.height=4, fig.width=4}
this_max_minus_min <- 5.455
this_num_spp <- 8
all_data %>%
  filter(max_minus_min == this_max_minus_min,
...
(ref:XXX14) Communities varying both the range and the amount of overlap, but with constant mean trait value.

```{r XXX14,  include=TRUE, fig.cap='(ref:XXX9)', fig.align="center", fig.height=4, fig.width=4}
this_mean <- 2.5
this_num_spp <- 8
all_data %>%
  filter(mean == this_mean,
...
(ref:XXX15) Communities with different numbers of species, but same range of trait values, same amount of overlap with zero, and same mean trait values.

```{r XXX15,  include=TRUE, fig.cap='(ref:XXX10)', fig.align="center", fig.height=4, fig.width=4}
this_mean <- 0
this_num_spp <- 8
this_max_minus_min <- 5.455
all_data %>%
...
(ref:XXX16) Communities with different numbers of species, but identical trait values.

```{r XXX16,  include=TRUE, fig.cap='(ref:XXX10)', fig.align="center", fig.height=4, fig.width=4}

num_spp <- c(2, 4, 8, 16,3)
redundant<-data.frame('S'=num_spp %>% rep(times=c(2,4,8,16,3)),'first_deriv_vals'=NA,'entropy'=NA)
...
for(i in 1:length(redundant$S)){
  ...
  redundant$entropy[i]<-first_deriv_vals %>% resp_fd()
}

redundant<-redundant %>%
  pivot_longer(names_to = "variable", values_to = "value", 2:3)
redundant$variable<-redundant$variable %>% 
  as.character() %>% parse_factor(levels = c('first_deriv_vals','entropy'))
